...
pokemon_data['results'].each do |pokemon|
  ...
  type2 = Type.where(name: type2_name).limit(1).pluck(:id).first if type2_name.present?
  type2 = nil unless type2_name.present?

  # Create the pokemon record
  Pokemon.create(
    name: name,
    type1: type1, # Assign the type1 ID directly
    type2: type2, # Assign the type2 ID directly or set it as nil
    HP: hp,
    ATK: attack,
    DEF: defense,
    SA: special_attack,
